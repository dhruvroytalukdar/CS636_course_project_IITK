...
//
//   void __wcp_read (void* addr, int32_t line_no, char* var_name)
//   void __wcp_write(void* addr, int32_t line_no, char* var_name)
...
struct WCPPass : public PassInfoMixin<WCPPass> {
    ...
    PreservedAnalyses run(Module &M, ModuleAnalysisManager &) {
        ...
        // void __wcp_read(void* addr, int line_no, char* var_name)
        FunctionCallee WcpRead =
            M.getOrInsertFunction("__wcp_read", VoidTy, VoidPtrTy, Int32Ty, VoidPtrTy);

        // void __wcp_write(void* addr, int line_no, char* var_name)
            ...
            for (BasicBlock &BB : F) {
                for (Instruction &I : BB) {
                    ...
                    if (auto *LI = dyn_cast<LoadInst>(&I)) {
                        ...
                        // Call Runtime
                        B.CreateCall(WcpRead, {Ptr, LineArg, NamePtr});
                        continue;
                    }
                    ...
                    if (auto *SI = dyn_cast<StoreInst>(&I)) {
                        ...
                        // Call Runtime
                        B.CreateCall(WcpWrite, {Ptr, LineArg, NamePtr});
                        continue;
                    }
