...
function usage {
    ...
    echo
    echo "Runs bug 11 to get timing results."
...
SHARED_IP_DIR=/tmp/apache-$BUG_NAME-sharedip
mkdir -p $SHARED_IP_DIR
SHARED_IP_OUT=$SHARED_IP_DIR/ip

function shmem {
    BENCH_FLAG_PRE="/usr/bin/time -v -o"
    BENCH_FLAG_POST="pin -follow_execv -t $SHMEM_TOOL -is_apache -o $SHARED_IP_OUT --"
