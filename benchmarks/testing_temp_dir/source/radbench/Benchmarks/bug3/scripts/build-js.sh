...
mkdir -p $NSPR_TARGET
cd $NSPR_TARGET
../mozilla/nsprpub/configure --prefix=$INSTALL_DIR
make
make install
...
cd $JS_SRC_SRC
PATH=${INSTALL_DIR}/bin:${PATH} XCFLAGS="$(nspr-config --cflags)" XLDFLAGS="$(nspr-config --libs)" JS_THREADSAFE=1 make -f Makefile.ref
echo "JS built"
