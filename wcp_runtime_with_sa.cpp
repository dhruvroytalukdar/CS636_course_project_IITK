...
// ── Unique race tracking ──────────────────────────────────────────────────────
struct RaceKey {
    int         line_no;
    std::string type;
    bool operator==(const RaceKey& o) const {
        return line_no == o.line_no && type == o.type;
    }
};
struct RaceKeyHash {
    size_t operator()(const RaceKey& k) const {
        return std::hash<int>{}(k.line_no) ^ (std::hash<std::string>{}(k.type) << 16);
    }
};
static std::mutex                                       unique_races_mtx;
static std::unordered_map<RaceKey, int, RaceKeyHash>    unique_races;
...
static void report_race(const char* type, void* addr,
    ...
    RaceKey key{line_no, std::string(type)};
    std::lock_guard<std::mutex> lk(unique_races_mtx);
    auto [it, inserted] = unique_races.emplace(key, 1);
    if (!inserted) it->second++;
}

__attribute__((destructor(101)))
static void print_unique_race_summary() {
    std::vector<std::pair<RaceKey, int>> sorted(unique_races.begin(),
                                                unique_races.end());
    std::sort(sorted.begin(), sorted.end(),
              [](const auto& a, const auto& b) {
                  return a.first.line_no != b.first.line_no
                       ? a.first.line_no < b.first.line_no
                       : a.first.type    < b.first.type;
              });
    fprintf(stderr, "\n");
    fprintf(stderr, "╔══════════════════════════════════════════════════════════════╗\n");
    fprintf(stderr, "║          WCP+SA  –  UNIQUE RACE LOCATION REPORT             ║\n");
    fprintf(stderr, "╠══════════════════════════════════════════════════════════════╣\n");
    fprintf(stderr, "║  Total reports   : %-6d                                   ║\n",
            race_count.load());
    fprintf(stderr, "║  Unique locations: %-6d                                   ║\n",
            (int)sorted.size());
    fprintf(stderr, "╠══════════╦══════════╦════════════════════════════════════════╣\n");
    fprintf(stderr, "║ Line     ║ Type     ║ Times triggered                        ║\n");
    fprintf(stderr, "╠══════════╬══════════╬════════════════════════════════════════╣\n");
    for (auto& [key, cnt] : sorted)
        fprintf(stderr, "║ %-8d ║ %-8s ║ %-38d ║\n",
                key.line_no, key.type.c_str(), cnt);
    fprintf(stderr, "╚══════════╩══════════╩════════════════════════════════════════╝\n");
    fprintf(stderr, "\n[UNIQUE_RACES] count=%d\n", (int)sorted.size());
    for (auto& [key, cnt] : sorted)
        fprintf(stderr, "[UNIQUE_RACE] LINE=%d TYPE=%s TRIGGERS=%d\n",
                key.line_no, key.type.c_str(), cnt);
}
