...
struct spin_lck {
    ...
    void lock() {
        // Exchange atomically sets to true and returns previous value.
        // If previous was false, loop is bypassed. Lock acquired.
        while(flag.exchange(true, std::memory_order_acquire)){
            // Pure read: wait for it to become false without modifying it.
            // Exiting this inner loop guarantees the outer exchange gets evaluated again.
            while (flag.load(std::memory_order_relaxed)) {
                _mm_pause();
            }
        }
...
//     void lock() {
//         while (true) {
//             bool is_set = flag.test(std::memory_order_relaxed);
//             if (is_set) {
//         //        _mm_pause();
//                 continue;
//             }
...
extern "C" void __log_store(void* x){
 ...
 //   std::cout << "debug:" << "got in a store at addr: " << x << std::endl;
    ...
    var->reader = empty_epoch; // Unconditional clear
    // var->writer = current_epoch;
