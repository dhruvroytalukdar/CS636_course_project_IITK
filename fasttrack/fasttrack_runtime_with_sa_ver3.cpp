...
void set_epoch_vector(std::vector<Epoch>& vec, int index, Epoch value){
    if(index >= (int)vec.size())
        vec.resize(index+1,0);
    vec[index] = value;
}

void set_access_vector(std::vector<int>& vec, int index, int value){
    if(index >= (int)vec.size())
        vec.resize(index+1,0);
    vec[index] = value;
}
...
bool is_ordered(ThreadState* t, int owner_tid, int owner_epoch) {
    ...
    return t->get_clock_of(owner_tid) >= owner_epoch;
}

bool can_reclaim_ownership(ThreadState* t,
                           const std::vector<int>& access_vc) {
    for (size_t i = 0; i < access_vc.size(); ++i) {
        int uid = i;
        int uepoch = access_vc[i];
        if (uid == t->tid) continue;
        if (t->get_clock_of(uid) < uepoch) return false;
    }
    return true;
}
...
bool __ft_read_client(void* addr, char* inst_str,
    ...
    if (x->R == READ_SHARED) {
        ...
        // if(t->tid >= (int)x->Rvc.size()){
        //     x->Rvc.resize(t->tid+1,0);
        // }
        // x->Rvc[t->tid] = t->epoch;
        set_epoch_vector(x->Rvc, t->tid, t->epoch);
    } else {
        ...
        if (r_clock <= t->get_clock_of(r_tid)) {
            ...
            // x->Rvc.clear();
            std::fill(x->Rvc.begin(), x->Rvc.end(), 0);
            set_epoch_vector(x->Rvc, r_tid, x->R);
            // x->Rvc[r_tid]  = x->R;
            set_epoch_vector(x->Rvc, t->tid, t->epoch);
            // x->Rvc[t->tid] = t->epoch;
            x->R = READ_SHARED;
...
bool __ft_write_client(void* addr, char* inst_str,
                       VarState* x, ThreadState* t,
                       std::unique_lock<std::recursive_mutex>& var_lock,
                       std::unique_lock<std::recursive_mutex>& thread_lock) {
    ...
    if (x->R != READ_SHARED) {
        if (x->R != 0) {
            ...
            if (r_clock > t->get_clock_of(r_tid)) {
                ...
                x->W = t->epoch;   // ← ADD THIS
                x->R = 0;          // ← ADD THIS: clear the read that raced
                return true;
            }
        }
    } else {
        for (int i = 0; i < x->Rvc.size(); i++) {
            auto u_tid = i;
            auto u_epoch = x->Rvc[i];
            int u_clock = get_clock(u_epoch);
            if (u_clock > t->get_clock_of(u_tid)) {
                report_race("R-W", addr, u_tid, t->tid, inst_str);
                x->W = t->epoch;   // ← ADD THIS
                std::fill(x->Rvc.begin(), x->Rvc.end(), 0);  // ← ADD THIS
                return true;
            }
        }
    }
...
void __ft_read(void* addr, char* inst_str) {
    ...
    // ── LOCK-FREE FAST PATH ────────────────────────────────────────
    ShareState current_state = x->share_state.load(std::memory_order_acquire);
    if ((current_state == ShareState::OWNED_READ || current_state == ShareState::OWNED_WRITE)
        && (x->owner_tid.load(std::memory_order_relaxed) == t->tid) && (x->R == t->epoch)) {
        return;
    }
    ...
    switch (x->share_state.load(std::memory_order_acquire)) {
        ...
        case ShareState::UNACCESSED:
            // First ever access is a READ
            x->owner_tid.store(t->tid, std::memory_order_relaxed);
            // x->owner_epoch = t->C[t->tid];
            x->owner_epoch = get_clock(t->epoch);
            x->share_state.store(ShareState::OWNED_READ, std::memory_order_release);
            return;

        case ShareState::OWNED_READ:
        case ShareState::OWNED_WRITE: {
            if (x->owner_tid.load() == t->tid) return;

            // Remove the is_ordered fast-return — it skips the W-R check
            int owner_tid_val   = x->owner_tid.load();
            int current_o_epoch = x->owner_epoch;
            ShareState prev_state = x->share_state.load();

            set_access_vector(x->access_vc, owner_tid_val, current_o_epoch);
            set_access_vector(x->access_vc, t->tid, get_clock(t->epoch));
            x->share_state.store(ShareState::SHARED, std::memory_order_release);

            if (prev_state == ShareState::OWNED_WRITE) {
                x->W = make_epoch(owner_tid_val, current_o_epoch);
                x->R = 0;
                std::fill(x->Rvc.begin(), x->Rvc.end(), 0);
            } else {
                x->W = 0;
                x->R = make_epoch(owner_tid_val, current_o_epoch);
                std::fill(x->Rvc.begin(), x->Rvc.end(), 0);
            }
        ...
        case ShareState::SHARED: {
            // if (t->tid >= (int)x->access_vc.size()) {
            //     x->access_vc.resize(t->tid + 1, 0);
            // }
            // x->access_vc[t->tid] = t->C[t->tid];
            // set_access_vector(x->access_vc, t->tid, get_clock(t->epoch));
            int recorded = (t->tid < (int)x->access_vc.size()) ? x->access_vc[t->tid] : 0;
            if (get_clock(t->epoch) > recorded)
                set_access_vector(x->access_vc, t->tid, get_clock(t->epoch));
            __ft_read_client(addr, inst_str, x, t, var_lock, thread_lock);
            break;
        }
...
void __ft_write(void* addr, char* inst_str) {
    ...
    // ── LOCK-FREE FAST PATH ────────────────────────────────────────
    ShareState current_state = x->share_state.load(std::memory_order_acquire);
    if (current_state == ShareState::OWNED_WRITE
        && (x->owner_tid.load(std::memory_order_relaxed) == t->tid) && (x->W == t->epoch)) {
        return;
    }
    ...
    switch (x->share_state.load(std::memory_order_acquire)) {
        ...
        case ShareState::UNACCESSED:
            ...
            // x->owner_epoch = t->C[t->tid];
            x->owner_epoch = get_clock(t->epoch);
            x->share_state.store(ShareState::OWNED_WRITE, std::memory_order_release);
            return;
        ...
        case ShareState::OWNED_WRITE: {
            if (x->owner_tid.load() == t->tid) {
                x->share_state.store(ShareState::OWNED_WRITE, std::memory_order_release);
                // x->owner_epoch = t->C[t->tid]; // Mark the write time
                x->owner_epoch = get_clock(t->epoch);
                return;
            }
            ...
            // No HB -> shared
            // if (owner_tid_val >= (int)x->access_vc.size()) {
            //     x->access_vc.resize(owner_tid_val + 1, 0);
            // }
        ...
        case ShareState::SHARED: {
            ...
            //     x->owner_tid.store(t->tid, std::memory_order_relaxed);
            //     // x->owner_epoch = t->C[t->tid];
            //     x->owner_epoch = t->epoch;
                
            //     std::fill(x->access_vc.begin(), x->access_vc.end(), 0);
            ...
            if (can_reclaim_ownership(t, x->access_vc)) {
                ...
                bool raced = __ft_write_client(addr, inst_str, x, t, var_lock, thread_lock);
                // Only reclaim SA ownership if FT did NOT report a race (W was updated)
                if (!raced) {
                    x->share_state.store(ShareState::OWNED_WRITE, std::memory_order_release);
                    x->owner_tid.store(t->tid, std::memory_order_relaxed);
                    x->owner_epoch = get_clock(t->epoch);
