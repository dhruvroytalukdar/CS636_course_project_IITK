...
struct VarState {
    ...
    //    Use atomics so the OWNED+same-thread fast path is lock-free
    ...
    // thread. NOT updated on every same-thread access (that was the key waste).
    int owner_epoch { 0 };

    // When SHARED: track the latest access epoch per thread so we can
    // detect when a writer has seen all prior accessors (collapse back to OWNED).
    ...
    // Full mutex: only acquired on slow/shared paths
    std::recursive_mutex mtx;
};
...
VarState* get_var_state(void* addr) {
    ...
    // auto& sv = get_shadow_vars();
    // if (sv.find(addr) == sv.end()) sv[addr] = new VarState();
    // return sv[addr];
...
void __ft_read(void* addr, char* inst_str) {
    ...
    // Atomic loads → no mutex, no write.
    // owner_epoch is NOT updated here — we only need it when a
    // *different* thread accesses and we must call is_ordered().
    if (x->share_state.load(std::memory_order_acquire) == ShareState::OWNED
        && x->owner_tid.load(std::memory_order_relaxed) == t->tid) {
        return;
    }

    // ── SLOW PATH: acquire locks and handle all other cases ────────
    std::unique_lock<std::recursive_mutex> var_lock(x->mtx);
    std::unique_lock<std::recursive_mutex> thread_lock(t->mtx);

    // Re-check under lock (state may have changed between fast-path check and here)
    ...
    case ShareState::OWNED: {
        if (x->owner_tid.load() == t->tid) {
            ...
            // NOTE: we deliberately do NOT update owner_epoch here.
            // It will be read fresh from the owner's ThreadState when needed.
            return;
        }

        // Different thread → check HB ordering using owner's current clock
        // (lazy: read it now rather than caching on every access)
        int owner_tid_val   = x->owner_tid.load();
        int current_o_epoch = x->owner_epoch; // last recorded at ownership time

        if (is_ordered(t, owner_tid_val, current_o_epoch)) {
            // HB edge exists → safe ownership transfer, no race possible
            x->owner_tid.store(t->tid, std::memory_order_relaxed);
            x->owner_epoch = t->C[t->tid];
            return;  // FastTrack never called
        }

        // No HB → truly shared for the first time: promote and init FastTrack
